...
describe("Navigation Component", () => {
  ...
  it("<a>要素がデフォルトで表示", () => {
    ...
    expect(linkElement).toHaveClass("text-info", "hover:underline", "hover:cursor-pointer");
  });
  ...
  it("asChildプロップがtrueの場合、任意のコンポーネントが表示", () => {
    ...
    // デフォルトのクラスが適用されていることを確認
    expect(buttonElement).toHaveClass("text-info", "hover:underline", "hover:cursor-pointer");
  });

  it("追加のclassNameとhrefを指定", () => {
    render(
      <Navigation href="/test" className="custom-class" data-testid="navigation">
        Link with Props
      </Navigation>
    );

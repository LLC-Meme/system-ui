...
describe("Stackコンポーネント", () => {
  ...
  describe("Stack.Vコンポーネント(縦スタック)", () => {
    ...
    it("gap=mdの場合、gap-8クラスが適用される", () => {
      render(<V data-testid="stack-v-md" gap="md">内容</V>);
    ...
    it("refを正しくforwardする", () => {
      ...
      render(<V ref={ref} data-testid="stack-v-ref">内容</V>);
  ...
  describe("Stack.Hコンポーネント(横スタック)", () => {
    ...
    it("gap=lgの場合、gap-12クラスが適用される", () => {
      render(<H data-testid="stack-h-lg" gap="lg">内容</H>);
    ...
    it("refを正しくforwardする", () => {
      const ref = React.createRef<HTMLDivElement>();
      render(<H ref={ref} data-testid="stack-h-ref">内容</H>);
      expect(ref.current).toBeInstanceOf(HTMLDivElement);
    });
  });

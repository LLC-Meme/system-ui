...
describe("Spacer.V", () => {
  it("表示される", () => {
    render(<VSpacer data-testid="spacer-v" />);
  ...
  it("unit=3で'h-12'が適用される", () => {
    render(<VSpacer data-testid="spacer-v" unit={3} />);
    const spacerEl = screen.getByTestId("spacer-v");
  ...
  it("デフォルトと追加のclassが適用", () => {
    render(
      <VSpacer data-testid="spacer-v" className="custom-class" unit={2} />,
    );
    const spacerEl = screen.getByTestId("spacer-v");
  ...
  it("refが適用", () => {
    ...
    render(<VSpacer ref={ref} data-testid="spacer-v" />);
...
describe("Spacer.H", () => {
  it("表示される", () => {
    render(<HSpacer data-testid="spacer-h" />);
  ...
  it("unit=5で'h-20'が適用される", () => {
    render(<HSpacer data-testid="spacer-h" unit={5} />);
    const spacerEl = screen.getByTestId("spacer-h");
    expect(spacerEl).toHaveClass("w-20");
  });

  it("デフォルトと追加のclassが適用", () => {
    render(<HSpacer data-testid="spacer-h" className="custom-h" unit={2} />);
    const spacerEl = screen.getByTestId("spacer-h");
  ...
  it("refが適用", () => {
    const ref = React.createRef<HTMLDivElement>();
    render(<HSpacer ref={ref} data-testid="spacer-h" />);
    expect(ref.current).toBeInstanceOf(HTMLDivElement);
  });
});
